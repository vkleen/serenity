...
//! where possible.
//! Issuing too many REST API requests will lead to ratelimits.
//!
...
pub trait FromStrAndCache: Sized + Send + Sync{
    ...
    async fn from_str<CRL: AsRef<Cache> + Send + Sync>(cache: CRL, s: &str) -> Result<Self, Self::Err>;
}
#[async_trait]
pub trait StrExt: Sized + Send + Sync {
    async fn parse_cached<CRL: AsRef<Cache> + Send + Sync, F: FromStrAndCache + Send + Sync>(&self, cache: CRL) -> Result<F, F::Err>;
}

#[async_trait]
impl<'a> StrExt for &'a str {
    async fn parse_cached<CRL: AsRef<Cache> + Send + Sync, F: FromStrAndCache + Send + Sync>(&self, cache: CRL)
    -> Result<F, F::Err> {
...
impl<F: FromStr + Send + Sync> FromStrAndCache for F {
    ...
    async fn from_str<CRL: AsRef<Cache> + Send + Sync>(_cache: CRL, s: &str) -> Result<Self, Self::Err> {
        s.parse::<F>()
    }
}
...
pub struct Cache {
    ...
    pub(crate) current_user: RwLock<CurrentUser>,
